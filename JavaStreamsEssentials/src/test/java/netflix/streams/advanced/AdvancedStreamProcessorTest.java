...
class AdvancedStreamProcessorTest {
    ...
    // ========== REACTIVE STREAM CREATION TESTS ==========

    @Test
    @DisplayName("Should create reactive stream with backpressure handling")
    void testCreateReactiveStream_Success() {
        // Given
        int bufferSize = 100;

        // When
        ReactiveStream<String> reactiveStream = AdvancedStreamProcessor.createReactiveStream(testData, bufferSize);

        // Then
        assertThat(reactiveStream).isNotNull();
        assertThat(reactiveStream.getBackpressureLevel()).isBetween(0.0, 1.0);
        assertThat(reactiveStream.getProcessingRate()).isGreaterThanOrEqualTo(0.0);
    }

    @Test
    @DisplayName("Should throw IllegalArgumentException for invalid buffer size")
    void testCreateReactiveStream_InvalidBufferSize() {
        // Test zero buffer size
        assertThatThrownBy(() -> AdvancedStreamProcessor.createReactiveStream(testData, 0))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("Buffer size must be positive: 0");

        // Test negative buffer size
        assertThatThrownBy(() -> AdvancedStreamProcessor.createReactiveStream(testData, -1))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("Buffer size must be positive: -1");
    }

    // ========== MEMORY EFFICIENT STREAM CREATION TESTS ==========

    @Test
    @DisplayName("Should create memory efficient stream")
    void testCreateMemoryEfficientStream_Success() {
        // When
        MemoryEfficientStream<String> memoryStream = AdvancedStreamProcessor.createMemoryEfficientStream(testData);

        // Then
        assertThat(memoryStream).isNotNull();
    }

    @Test
    @DisplayName("Should throw IllegalArgumentException for null source")
    void testCreateMemoryEfficientStream_NullSource() {
        assertThatThrownBy(() -> AdvancedStreamProcessor.createMemoryEfficientStream(null))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("Source collection cannot be null");
    }
