# Distributed Locking - Netflix Production Guide

## Concept overview

Distributed locks coordinate access to shared resources across processes and hosts. Production-grade locking must be fair, fault tolerant, and safe under process crashes and network partitions. Prefer idempotency and conflict free designs first; use locks only where necessary.

## Implementation layer classification

| Component | Layer | Type | Netflix status |
|---|---|---|---|
| Redis lock | Application | SET NX PX + Lua | Production |
| Zookeeper lock | Application plus infrastructure | Ephemeral sequential znode | Production |
| Fencing tokens | Application | Monotonic tokens | Production |
| Heartbeats | Application | Session renewal | Production |

## Patterns

- Use short TTLs and renew while holding the lock
- Attach an owner id and use Lua to release only if owner matches
- Use fencing tokens to prevent stale owners from acting after expiry
- Prefer Curator recipes for Zookeeper for correctness

## Usage example

```java
// Redis
try (var lock = redisLock.tryLock("inventory:123", Duration.ofSeconds(5))) {
    if (lock.acquired()) {
        // critical section
    }
}
...
// Zookeeper (Curator)
try (var lock = zkLock.tryLock("/locks/inventory-123", Duration.ofSeconds(10))) {
    if (lock.acquired()) {
        // critical section
    }
}
...
- Lock leaks: ensure try-with-resources or finally unlock
- Split brain: use fencing tokens and verify on write path
- Timeouts: adjust TTL and renew interval; monitor latency

## Threat model and failure modes

- Process pauses and network delays: require fencing tokens at the write path
- Split brain with approximate stores (single Redis): acceptable only for efficiency locks
- Lost renewals: treat as lease loss; ensure idempotent critical sections
- Clock skew: avoid time-based correctness assumptions

## Established libraries and when to use

- ZooKeeper/Curator recipes: for correctness critical locks with fencing via zxid/version
- etcd concurrency: lease based locks with strong semantics
- Redis single node with Lua: efficiency locks only; document approximation

## Decision tree (summary)

- Correctness critical?
  - Yes: use ZooKeeper/etcd; implement fencing tokens on resources
  - No: Redis single node acceptable; short TTL and renewer
- Needs cross DC?
  - Prefer consensus system colocated with writers; avoid WAN lock managers

## Production readiness checklist

- Fencing tokens enforced on all writes under lock
- Short TTL + renewer with backoff; unlock in finally/try-with-resources
- Metrics: lock acquire latency, renew success rate, lease expiries, contention
- Alerts: renew failure spikes, contention > threshold, dual-writer detection

### Quantified trade offs
* Redis single node efficiency locks: SET NX PX adds 0.1 to 0.5 ms intra AZ per op. Lua release adds 0.1 to 0.3 ms. With 1 percent to 5 percent probability of approximate failure under node crash windows equal to TTL.
* TTL and renew interval: set TTL at 5x to 10x of expected critical section time. Renew interval at 0.2x to 0.3x TTL to keep margin for GC pauses. If P99 GC pause is 150 ms, choose renew 200 to 300 ms and TTL 1 to 1.5 s.
* Fencing tokens: monotonic counter allocation cost 0.1 to 0.5 ms in ZK or etcd. Storage layer token check adds 0.05 to 0.2 ms per write and eliminates stale writer risk.
* Zookeeper Curator mutex: acquire latency 2 to 10 ms p50, 5 to 50 ms p99 under contention due to ZAB quorum and sequential znodes. Throughput decreases linearly with contention; keep concurrent waiters under a few hundred per lock path.
* etcd lease locks: grant and keepalive RPCs add 0.3 to 1 ms intra AZ. Keepalive at 1 second interval with 3 second TTL is typical for fast failure detection.
* Dual writer detection: external store compare and set with version increments adds 0.2 to 1 ms per guarded write. Budget per write path accordingly.

## References

- Redlock analysis and guidance
- Apache Curator recipes


# We didn't consider or care with any of these 
