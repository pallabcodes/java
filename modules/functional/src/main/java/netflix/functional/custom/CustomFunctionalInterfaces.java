...
public class CustomFunctionalInterfaces {
    ...
    public interface ThrottledFunction<T, R> {
        ...
        default ThrottledFunction<T, R> withThrottling(long throttleMillis) {
            java.util.concurrent.atomic.AtomicLong lastExecution = new java.util.concurrent.atomic.AtomicLong(0);
            return input -> {
                long currentTime = System.currentTimeMillis();
                if (currentTime - lastExecution.get() >= throttleMillis) {
    ...
    public interface DebouncedFunction<T, R> {
        ...
        default DebouncedFunction<T, R> withDebouncing(long debounceMillis) {
            java.util.concurrent.atomic.AtomicReference<Timer> timerRef = new java.util.concurrent.atomic.AtomicReference<>(new Timer());
            return input -> {
                Timer oldTimer = timerRef.getAndSet(new Timer());
                if (oldTimer != null) {
                    oldTimer.cancel();
                }
                timerRef.get().schedule(new TimerTask() {
                    @Override
                    public void run() {
