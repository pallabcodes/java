...
public class BoundsExamples {
    ...
    /**
     * Demonstrates lower bounds with generic methods
     * 
     * Lower bounds restrict the type parameter to be a supertype of the specified type.
     * 
     * @param list the list to add numbers to
     * @param <T> the type parameter bounded by Integer
     */
    public static <T super Integer> void processLowerBound(List<T> list) {
        log.info("Processing lower bound list with {} elements", list.size());
        
        // Can add elements
        list.add((T) Integer.valueOf(42));
        list.add((T) Integer.valueOf(100));
        list.add((T) Integer.valueOf(200));
